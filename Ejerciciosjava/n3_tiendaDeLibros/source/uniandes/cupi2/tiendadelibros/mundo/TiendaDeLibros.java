...
public class TiendaDeLibros
{
    ...
    {	
    	...
    	{
    		Libro actual = catalogo.get(i);
    		//Recorre el arreglo con los limites inferiores y superiores
    		for (int j = 0; j < lim_inf.length; j++)
    		{
    			// Se pregunta si el libro actual se encuentra en el rango
    			if (lim_inf[j] <= actual.darPrecioVenta() && actual.darPrecioVenta() < lim_sup[j] )
    			{
    				// Aplica el descuento dependiendo del rango que se encuentra el precio. 
    				double precio = actual.darPrecioVenta();
    				actual.cambiarPrecioVenta( precio * ( 1 - descuentos[j]) );  
    			}
    		}
    	}
    ...
    public Transaccion metodo2()
    {
    	...
    	for ( Libro libroActual: catalogo )
    	{
    		// Guarda las transacciones del libro actual
    		ArrayList<Transaccion> transacciones = libroActual.darTransacciones();
    		for ( int i = 0; i < transacciones.size(); i++)
    		{
    			// i-esima transaccion del libro actual 
    			Transaccion transaccionActual = transacciones.get(i);
    			// i-esimo valor de la transaccion del libro actual
    			double valorTransaccionActual = libroActual.darValorTransacciones().get(i);
    			// verifica que la transaccion sea tipo venta
    			if( transaccionActual.darTipo( ).equals( Transaccion.Tipo.VENTA ) )
    			{
    				//verifica si la transaccion actual es mas costosa y la guarda. 
    				if (valorTransaccion < valorTransaccionActual)
    				{
    					valorTransaccion = valorTransaccionActual;
    					masCostosa = transaccionActual;
    				}	
    			}
    		}	
    	}

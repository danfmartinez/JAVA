...
public class Pasajero
{
    ...
    {
    	//TODO Implemente el método según la documentación.
    	if ( (nombre != "" && nombre != null) && (genero != "" && genero != null) 
    			&& ( duracion > 0) && (director != null && director != "") )
    	{
    ...
    public int darDuracionTotalPeliculas()
    {
    	...
    	{
    		suma += peliculas.get(i).darDuracion(); 
    	}
    ...
    {
    	//TODO Implemente el método según la documentación.
    	ArrayList<Pelicula> listaGenero = new ArrayList<Pelicula> () ; 
    	for ( int i = 0; i < peliculas.size(); i++ )
    	{
    		if (peliculas.get(i).darGenero().equals(genero) )
    		{
    			listaGenero.add( peliculas.get(i) );
    		}
    	}
    	return listaGenero; 
    }
    ...
    public boolean tienePeliculasMismaDuracion()
    {
    	...
    	for (int i = 0; i < peliculas.size() && !iguales; i++ )
    	{
    		for ( int j = i+1; j < peliculas.size() && !iguales; j++ )
    		{
    			if (i != j && ( peliculas.get(i).darDuracion() == peliculas.get(j).darDuracion() ) )
    				{
    				  iguales = true ;
    				}
    		}

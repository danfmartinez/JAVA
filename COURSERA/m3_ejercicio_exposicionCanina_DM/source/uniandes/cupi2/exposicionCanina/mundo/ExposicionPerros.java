...
public class ExposicionPerros
{
    ...
    public void ordenarPorRaza( )
    {
    	...
    	perritos = darPerros();
    	Perro temp;
    	for (int i = perritos.size(); i > 0; i-- )
    	{
    		for(int j = 0; j < i - 1; j++)
    		{
    			if (perritos.get(j).compararPorRaza( perritos.get(j+1)) == 1)
    			{
    				temp = perritos.get(j);
    				perritos.set(j, perritos.get(j+1));
    				perritos.set(j+1, temp);
    			}
    		}
    	}
    	for (int n = 0; n < perros.size(); n++)
    	{
    		perros.set(n, perritos.get(n));
    	}
    }
    ...
    public void ordenarPorNombre( )
    {
    	for(int i = 0; i < perros.size(); i++)
    	{
    		for(int j = i; j > 0; j-- )
    		{
    			Perro anterior = (Perro) perros.get(j-1);
    			Perro actual = (Perro) perros.get(j);
    			if(anterior.compararPorNombre(actual) == 1)
    			{
    				perros.set(j, anterior);
    ...
    public void ordenarPorPuntos( )
    {
    	for(int i = perros.size(); i > 0; i-- )
    	{
    		...
    		{
    			Perro actual = (Perro) perros.get(j);
    			Perro porInsertar = (Perro) perros.get(j+1);
    			if(actual.compararPorPuntos( porInsertar ) == 1)
    			{
    				perros.set(j, porInsertar );
    				perros.set(j+1, actual);
    			}
    		}
    	}  
    ...
    public void ordenarPorEdad( )
    {
    	for (int i = 0; i < perros.size() - 1; i++ )
    	{
    		...
    		{
    			Perro actual = (Perro) perros.get(j);
    			if (actual.compararPorEdad(menor) == -1 )
    			{
    				menor = (Perro) perros.get(j);
    				cual = j;
    			}
    		}
    		Perro temp = (Perro)perros.get(i);
    		perros.set(i, menor);
    		perros.set(cual,temp);
    	}
